...
class UserRepository(
    ...
    suspend fun getUserStats(userId: Long): Result<UserStatsDto> {
        val tokenDataStore = tokenStoreManager.accessTokenFlow.first()
        val token = "Bearer $tokenDataStore"
        return try {
            Log.d("UserRepository", "Đang lấy tương tác của user")
            val response = userApiService.getUserStats(token, userId)
            Log.d("UserRepository", "Response stats: $response")
            if (response.isSuccessful) {
                val userStats = response.body()
                Log.d("UserRepository", "Tương tác của user: $userStats")
                if (userStats != null) {
            ...
            MediaManager.get().upload(imageBytes)
                ...
                .callback(object : UploadCallback {
                    ...
                    override fun onSuccess(requestId: String?, resultData: Map<*, *>?) {
                        ...
                        val resizedUrl =
                            originalUrl?.replace("/upload/", "/upload/w_800,h_600,c_limit/")
